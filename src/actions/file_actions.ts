...
export const change_type_action = (type: String) => async (
...
export const upload_markdown_action = (note: FileObject) => async (
  dispatch: Dispatch<Action>
): Promise<void> => {
  ...
  if (response.status === 201) {
    ...
    dispatch({ type: TOKEN_EXP, payload: response });
  }
};

export const auto_save_note = (note: FileObject, id: number) => async (
  dispatch: Dispatch<Action>
): Promise<void> => {
  dispatch({ type: LOADING_TRUE });
...
export const get_your_files_action = () => async (
  dispatch: Dispatch<Action>
): Promise<void> => {
  dispatch({ type: LOADING_TRUE });
...
export const delete_file_action = (id: number) => async (
  dispatch: Dispatch<Action>
): Promise<void> => {
  dispatch({ type: LOADING_TRUE });
